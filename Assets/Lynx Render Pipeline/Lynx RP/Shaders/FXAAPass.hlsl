...
float GetEdgeBlendFactor (LumaNeighborhood luma, FXAAEdge edge, float2 uv)
{
	...
	else
	{
		...
		// return 10.0 * (distanceToEndN - distanceToEndP);
		// return 10.0 * (distanceToEndP - distanceToEndN);
...
float4 FXAAPassFragment (Varyings input) : SV_TARGET 
{
	...
	// return blendFactor;
	// return float4(max(edge.pixelStep, 0.0), abs(min(edge.pixelStep, 0.0)), 0.0, 0.0);
