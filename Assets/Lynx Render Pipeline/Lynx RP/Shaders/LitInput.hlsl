...
UNITY_INSTANCING_BUFFER_START(UnityPerMaterial) //CBUFFER_START(UnityPerMaterial)
    ...
    UNITY_DEFINE_INSTANCED_PROP(float4, _BaseColor) //float4 _BaseColor;
...
float3 GetNormalTS (InputConfig c)
{
	...
	if (c.useDetail)
	{
		float4 mapDetail = SAMPLE_TEXTURE2D(_DetailNormalMap, sampler_DetailMap, c.detailUV);
		float scaleDetail = INPUT_PROP(_DetailNormalScale) * GetMask(c).b;
		float3 detail = DecodeNormal(mapDetail, scaleDetail);
